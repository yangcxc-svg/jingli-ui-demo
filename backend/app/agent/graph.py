...
class GuideAgent:
    ...
    async def run(
        ...
        prompt = self._build_prompt(message, chunks, image_ids or [])
        image_refs = self._build_image_refs(image_ids or [])
        result = await self.llm.generate(
            prompt, system=GUIDE_SYSTEM_PROMPT, images=image_refs, history=history
        )
    ...
    def _build_prompt(
        self, message: str, chunks: list[dict[str, object]], image_ids: list[str]
    ) -> str:
        parts: list[str] = []
        if chunks:
            joined = "\n".join(
            ...
            parts.append(
                "以下是可能与用户问题相关的知识库片段，请在回答时优先依据这些片段，"
                "并在合适处用 [编号] 标注引用：\n" + joined
            )
        if image_ids:
            names: list[str] = []
