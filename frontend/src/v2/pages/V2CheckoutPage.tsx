  ...
  return (
    <div className="min-h-full bg-[#f8f9fb] px-4 pb-28 pt-2 text-slate-950 animate-fadeIn">
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="text-[16px] font-black text-slate-900">送朋友</span>
        <div className="w-8" />
      </div>
      ...
      <div className="mb-5 flex items-center justify-center rounded-2xl bg-white px-4 py-3 shadow-sm ring-1 ring-slate-100">
        <div className="flex items-center gap-1.5 text-[12px]">
          <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#e4393c] text-[11px] font-black text-white">1</span>
          <span className="font-black text-[#e4393c]">下单支付</span>
          <span className="mx-1 text-slate-300">›</span>
          <span className="flex h-5 w-5 items-center justify-center rounded-full bg-slate-100 text-[11px] font-black text-slate-400">2</span>
          <span className="font-semibold text-slate-400">定制贺卡并分享</span>
          <span className="mx-1 text-slate-300">›</span>
          <span className="flex h-5 w-5 items-center justify-center rounded-full bg-slate-100 text-[11px] font-black text-slate-400">3</span>
      ...
      <div className="mb-4 space-y-2.5">
        <button
          ...
          className={`flex w-full items-center justify-between rounded-2xl px-4 py-3.5 text-left transition ${
            addressMode === 'friend'
              ? 'bg-[#fff0f2] ring-1 ring-[#e4393c]/20'
              : 'bg-white shadow-sm ring-1 ring-slate-100'
          }`}
        >
          <div className="flex items-center gap-2.5">
            <div className={`flex h-6 w-6 items-center justify-center rounded-full ${addressMode === 'friend' ? 'bg-[#e4393c]' : 'bg-slate-200'}`}>
              <Icon name="gift" className={`h-3 w-3 ${addressMode === 'friend' ? 'text-white' : 'text-slate-500'}`} />
            </div>
            <span className={`text-[14px] font-black ${addressMode === 'friend' ? 'text-[#e4393c]' : 'text-slate-700'}`}>
              朋友选地址
            </span>
          </div>
          <div className="flex items-center gap-1">
            <span className="text-[12px] text-slate-400">付款后好友填地址</span>
        ...
        >
          <div className="flex items-center gap-2.5">
            <div className={`flex h-6 w-6 items-center justify-center rounded-full ${addressMode === 'self' ? 'bg-[#e4393c]' : 'bg-slate-200'}`}>
              <Icon name="map-pin" className={`h-3 w-3 ${addressMode === 'self' ? 'text-white' : 'text-slate-500'}`} />
            </div>
            <span className={`text-[14px] font-black ${addressMode === 'self' ? 'text-[#e4393c]' : 'text-slate-700'}`}>
              我选地址
            </span>
          </div>
          {addressMode === 'self' ? (
            ...
            <span className="text-[12px] text-slate-400">选择收礼地址 ›</span>
          )}
        </button>
      </div>
      ...
      <div className="mb-4 rounded-[18px] bg-white p-4 shadow-sm ring-1 ring-slate-100">
        <h3 className="mb-3 text-[14px] font-black text-slate-900">礼物</h3>
        {cartItems.length === 0 ? (
          <p className="py-4 text-center text-[12px] text-slate-400">购物车为空</p>
        ) : (
            ...
            {displayGifts.map((it, idx) => (
              <div key={`${it.productId}-${idx}`} className="flex gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[14px] font-black leading-snug text-slate-900 line-clamp-2">{it.name}</p>
                  <p className="mt-1 text-[12px] text-slate-400 line-clamp-1">{it.desc}</p>
                  <p className="mt-2 text-[14px] font-black text-[#e4393c]">{formatYuan(it.price)}</p>
                </div>
              </div>
            ))}
            {cartItems.length > 1 && (
              <button
                ...
                className="mx-auto flex items-center gap-1 rounded-full bg-slate-50 px-4 py-1.5 text-[12px] font-semibold text-slate-500 transition hover:bg-slate-100"
              >
      ...
      <div className="mb-4 rounded-[18px] bg-white p-4 shadow-sm ring-1 ring-slate-100">
        <div className="space-y-3 text-[12px]">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-semibold text-[#e4393c]">-{formatYuan(discount)}</span>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <span className="text-slate-500">优惠券</span>
              <span className="rounded bg-[#fff0f2] px-1.5 py-0.5 text-[11px] font-black text-[#e4393c]">已选推荐优惠</span>
            </div>
            <span className="font-semibold text-[#e4393c]">-{formatYuan(couponDiscount)}</span>
          </div>
          <div className="flex items-center justify-between border-t border-slate-100 pt-3">
            <span className="text-[14px] font-black text-slate-700">合计</span>
      ...
      {/* 定制服务（对应步骤2 定制贺卡） */}
      <div className="mb-4 rounded-[18px] bg-white p-4 shadow-sm ring-1 ring-slate-100">
        <h3 className="mb-3 text-[14px] font-black text-slate-900">定制服务</h3>
        <div className="space-y-3">
          <label className="flex cursor-pointer items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-[13px] font-bold text-slate-700">奢华绒皮礼盒</p>
                <p className="text-[12px] text-slate-400">含香薰干花与金丝带</p>
              </div>
            </div>
            <input
              type="checkbox"
              ...
              className="h-5 w-5 accent-[#e4393c]"
            />
          </label>
          <label className="flex cursor-pointer items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-[13px] font-bold text-slate-700">AI 语音贺卡二维码</p>
                <p className="text-[12px] text-slate-400">包装印二维码，扫码听语音祝福</p>
              </div>
            </div>
            <input
              type="checkbox"
      ...
      <div className="fixed bottom-16 left-4 right-4 z-50 rounded-2xl bg-white px-4 py-3 shadow-[0_-8px_32px_rgba(15,23,42,0.12)]">
        <div className="mx-auto max-w-md">
          <div className="mb-2 flex items-baseline gap-2">
            <span className="text-[18px] font-black text-[#e4393c]">{formatYuan(totalAmount)}</span>
            <span className="text-[12px] text-slate-400">共减{formatYuan(discount + couponDiscount)}</span>
          </div>
          <div className="grid grid-cols-[1fr_1.6fr] gap-2">
            ...
            >
              <span className="text-[14px] font-black text-[#e4393c]">群送礼</span>
              <span className="text-[11px] text-[#e4393c]/70">{cartItems.length}份礼物，每人抽一份</span>
            </button>
            <button
              onClick={handlePay}
              disabled={submitting || cartItems.length === 0}
              className="flex items-center justify-center rounded-xl bg-gradient-to-r from-[#e4393c] to-[#e4393c] text-[14px] font-black text-white shadow-lg transition active:scale-95 disabled:opacity-50"
            >
