...
const initialJingliMessages: JingliChatMessage[] = [
  ...
  },
  {
    id: 'demo-user',
    role: 'user',
    content: '想给25岁女生送生日礼物，预算500元。',
  },
  {
    id: 'demo-assistant',
    role: 'assistant',
    content: '这个场景建议选择有心意、颜值高、不过度昂贵的礼物。可以优先考虑香氛礼盒、小家电、美妆护肤或精致数码配件。',
  },
];

function formatProductPrice(price: ProductCardData['price']) {
...
function BackendProductCard({
  ...
  const absoluteImage = product.image_url?.startsWith('http') ? product.image_url : null;
    ...
    <article className="w-[148px] flex-none rounded-2xl bg-white p-2 shadow-sm ring-1 ring-zinc-100">
      <div className="grid h-20 place-items-center overflow-hidden rounded-xl bg-gradient-to-br from-sky-50 via-violet-50 to-red-50">
        {absoluteImage && !imageFailed ? (
          <img src={absoluteImage} alt={product.name} className="h-full w-full object-cover" onError={() => setImageFailed(true)} />
        ) : (
          <span className="grid h-12 w-12 place-items-center rounded-2xl bg-white text-2xl shadow-sm">🎁</span>
        )}
      </div>
      <h4 className="mt-2 line-clamp-2 min-h-8 text-[12px] font-black leading-4 text-zinc-950">{product.name}</h4>
      ...
      <button
        ...
        className={`mt-2 h-7 w-full rounded-full text-[11px] font-black text-white shadow-sm transition active:scale-95 disabled:active:scale-100 ${
          added ? 'bg-emerald-500' : 'bg-[#e93b3d]'
        } ${adding ? 'opacity-70' : ''}`}
      >
        {adding ? '加入中' : added ? '已加入' : '加入礼单'}
      </button>
    </article>
  );
}
...
function JingliPage() {
  ...
  async function handleSend(message: string) {
    ...
    try {
      ...
      console.error(error);
      setErrorMessage('后端暂时没有响应，请确认 Ai_buy 后端已启动在 8000 端口。');
      setMessages((current) =>
        current.map((item) =>
  ...
  async function handleAddGiftList(product: ProductCardData) {
    ...
    try {
      ...
      console.error(error);
      setErrorMessage('加入礼单失败，请确认后端礼单接口已启动。');
    } finally {
